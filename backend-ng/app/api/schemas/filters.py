...
class AppFilter(Filter):
    ...
    @staticmethod
    def filter__is_available_in_countries(query: FindMany[FindType], value: str):
        return query.find_many(
            {f"prices.{country}.is_available": {'$eq': True} for country in value.split(',')}
        )

    @staticmethod
    def filter__discount_eq(query: FindMany[FindType], value: int):
        return query.find_many(
            {f"prices.{settings.MAIN_COUNTRY}.price_story.0.discount": {'$eq': value}}
        )

    @staticmethod
    def filter__discount_gte(query: FindMany[FindType], value: int):
        return query.find_many(
            {f"prices.{settings.MAIN_COUNTRY}.price_story.0.discount": {'$gte': value}}
        )

    @staticmethod
    def filter__discount_lte(query: FindMany[FindType], value: int):
        return query.find_many(
            {f"prices.{settings.MAIN_COUNTRY}.price_story.0.discount": {'$lte': value}}
        )

    @staticmethod
    def sort__discount(query: FindMany[FindType], direction: SortDirection):
        return query.sort((f"prices.{settings.MAIN_COUNTRY}.price_story.0.discount", direction))

    class Constants(Filter.Constants):
        model = App
        search_model_fields = ["name, short_description"]
